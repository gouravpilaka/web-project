...
if($table == "routine"){
	if($action == 'read'){
		if($_SESSION['user_type'] == "admin"){
			$result = $conn->query("SELECT * FROM `routine` r join user u on u.id = r.user_id");
		}else{
			$result = $conn->query("SELECT * FROM `routine` r join user u on u.id = r.user_id where user_id = $user_id");
		}
